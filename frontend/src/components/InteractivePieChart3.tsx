...
// Mock types for standalone artifact
interface Activity {
  _id: string;
  category?: string;
  subcategory?: string;
...
const InteractivePieChart = ({ activities, categories }: InteractivePieChartProps) => {
  ...
  // Auto-drill logic
  useEffect(() => {
    // Don't auto-drill if user manually navigated back
    if (manualNavigation) {
    ...
    if (drillLevel === 'category') {
      ...
      if (visibleSubcategories.length === 1 && visibleSubcategories[0].subcategory) {
        setDrilldownSubcategory(visibleSubcategories[0].subcategory);
        setDrillLevel('activity');
      }
    }
  }, [hiddenCategories, hiddenSubcategories, drillLevel, drilldownCategory, activities, manualNavigation]);
  ...
  const handleBackToSubcategories = () => {
    setManualNavigation(true);
    if (drilldownCategory && hasSubcategories(drilldownCategory)) {
      setDrillLevel('subcategory');
      setDrilldownSubcategory(null);
    } else {
  ...
  const handleBackNavigation = () => {
    if (drillLevel === 'activity') {
      // If we're at activity level, check if we came from subcategories or directly from categories
      if (drilldownSubcategory && drilldownCategory && hasSubcategories(drilldownCategory)) {
        // We came from subcategories, go back to subcategories
        handleBackToSubcategories();
      } else {
        // We came directly from categories (no subcategories), go back to categories
        handleBackToCategories();
      }
    } else if (drillLevel === 'subcategory') {
      // If we're at subcategory level, go back to categories
      handleBackToCategories();
    }
  };
          ...
          <div className="flex items-center gap-3">
            {drillLevel !== 'category' && (
              <button
                ...
                title={
                  drillLevel === 'activity' && drilldownSubcategory && drilldownCategory && hasSubcategories(drilldownCategory)
